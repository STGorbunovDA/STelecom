...
namespace STelecom.Classes.FormsMethods
{
    internal class WorkFormMethod
    {
        ...
        /// <summary>
        /// Сохранение Базы для дирекции связи в CSV in PC
        /// </summary>
        /// <param name="dgw"></param>
        /// <param name="cmbCity"></param>
        internal static void DirectorateSaveFilePC(DataGridView dgw, string cmbCity)
        {
            DateTime dateTime = DateTime.Now;
            string dateTimeString = dateTime.ToString("dd.MM.yyyy");
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Filter = "csv files (*.csv)|*.csv|All files (*.*)|*.*";
            sfd.FileName = $"База_{cmbCity}_{dateTimeString}";
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                using (StreamWriter sw = new StreamWriter(sfd.FileName, false, Encoding.Unicode))
                {
                    ...
                    sw.WriteLine(note);
                    string poligon = String.Empty; string company = String.Empty; string location = String.Empty; 
                    string model = String.Empty; string serialNumber = String.Empty; string inventoryNumber = String.Empty; 
                    string networkNumber = String.Empty; string dateTO = String.Empty; string numberAct = String.Empty;
                    string numberActRemont = String.Empty; string category = String.Empty; string price = String.Empty;
                    string decommissionSerialNumber = String.Empty; string priceRemont = String.Empty; 
                    string city = String.Empty; string comment = String.Empty;
                    for (int i = 0; i < dgw.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgw.ColumnCount; j++)
                        {
        ...
        /// <summary>
        /// Сохранение полной Базы в CSV in PC
        /// </summary>
        /// <param name="dgw"></param>
        /// <param name="cmbCity"></param>
        internal static void SaveFullBasePC(DataGridView dgw, string cmbCity)
        {
            DateTime dateTime = DateTime.Now;
            string dateTimeString = dateTime.ToString("dd.MM.yyyy");
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Filter = "csv files (*.csv)|*.csv|All files (*.*)|*.*";
            sfd.FileName = $"ОБЩАЯ База_{cmbCity}_{dateTimeString}";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                using (StreamWriter sw = new StreamWriter(sfd.FileName, false, Encoding.Unicode))
                {
